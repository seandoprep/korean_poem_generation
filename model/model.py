...
class ClipCaptionModel(nn.Module):
    ...
    def __init__(
        ...
        self.gpt = GPT2LMHeadModel.from_pretrained("sean3819/KoGPT2_poem_finetuning")
...
def predict(image, model):
    ...
    # pred_clip_model, pred_preprocess = clip.load("ViT-B/32", device=device, jit=False)
    pred_clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
    pred_preprocess = AutoProcessor.from_pretrained("openai/clip-vit-base-patch32")
    image = pred_preprocess(images=image, return_tensors="pt")# .unsqueeze(0).to(device)
    with torch.no_grad():
        prefix = pred_clip_model.get_image_features(**image).to(device, dtype=torch.float32)
        prefix_embed = model.clip_project(prefix).reshape(1, 10, -1)
    tokenizer = PreTrainedTokenizerFast.from_pretrained(
        "sean3819/KoGPT2_poem_finetuning"
    )
    entry_count = 1
    entry_length =8  # maximum number of words
    top_p = 0.8
    temperature = 1.0
    stop_token = " "
    model.eval()
    ...
    pred_gpt = GPT2LMHeadModel.from_pretrained("sean3819/KoGPT2_poem_finetuning_full") #change
    input_ids = torch.tensor(input_ids)[None].cuda() if torch.cuda.is_available() else torch.tensor(input_ids)[None].cpu()
    gen_ids = pred_gpt.generate(
        input_ids,
        max_length=64,
